...
首页点击「自动发现机会」，无需提供主题。复用 `.env` 中研究/翻译模型配置，后台自动选4个不同方向，分别检索 HN、GitHub Issues、Stack Overflow（每渠道每方向最多30条，过去两年）。同一时间只运行一轮，多次点击复用当前任务。

采集后按来源与正文去重，排除已关闭 Issue 和已回答问题；其余全部分批分析，每批30篇、每篇前4000字符，每批最多8条需求信号。再合并所有批次，经独立模型复核与发布规则检查，剔除无依据、重复或不适合单人落地的建议，给出0–5个按验证优先级排列的产品建议，包含付费者、未满足需求、最小方案、可行性、收费假设、竞争风险与下一步验证动作。引文由程序复制原文，模型引用ID受校验；采集失败、未分析数量、来源覆盖可见。报告是研究建议，不能代替实际付费验证。模型复核与规则都不能保证语义判断正确，当前规则另过滤机构采购、大范围方案和部分未经证实的强断言；宁可显示无合格结果。

三个渠道均偏技术社区，尚不代表全网市场。GitHub正文不含后续评论，HN缺完整父帖，SO不抓回答。来源市场成功和目标国家需求都缺证据时，不会仅凭美国→中东/欧洲/亚洲（不含中国）的偏好断言存在本地化机会。创始人资料可选；空白不阻塞发现。更新由按钮触发，不隐式定时消耗API。

API：`POST /api/radar` 启动或复用任务；`GET /api/radar` 查看最新阶段、覆盖与报告。任务与批次存入数据库，Worker重启可续跑，失效租约不能发布结果。
