...
2. 在「参考产品」导入产品官网 / 收费页；把用户评论、讨论等原文关联到产品。也可以创建多个同域名的不同产品。
3. 点击「研究机会」。后台先提取带原文行号的声明，再基于固定声明 ID 提出具体机会。研究复用 `.env` 的 `TRANSLATION_API_KEY` / `TRANSLATION_MODEL` / `TRANSLATION_BASE_URL`。
4. 在首页草稿打开机会，检查目标付费者、任务、市场缺口、最小方案、收费与获客假设、工期和维护风险。没有需求证据的明确待补证。
...
- 研究最多使用每个产品 6 个最新关联源，每篇前 12000 字符；模型输入标记截断。没有全网自动发现，没有伪造 SEO 或用户调研结果。
